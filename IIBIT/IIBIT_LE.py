...
# MAIN ROUTINE
course_links = []
each_url = 'https://www.iibit.edu.au/courses/'
domain_url = 'https://www.iibit.edu.au/'
...
p_tags = soup.find_all('p', {'class': 'course-box-bottom'})
if p_tags:
    for p in p_tags:
        a_tag = p.find('a', {'class': 'pull-right', 'href': True})
        if a_tag:
            link = a_tag['href']
            if link:
...
course_links_file_path = os.getcwd().replace('\\', '/') + '/iibit_links_file'
