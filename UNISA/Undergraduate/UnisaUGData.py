...
course_links_file_path = course_links_file_path.__str__() + '/unisa_undergrad_links_file_MAIN'
course_links_file = open(course_links_file_path, 'r')

# the csv file we'll be saving the courses to
csv_file_path = Path(os.getcwd().replace('\\', '/'))
csv_file = 'UNISA_Undergrad_Data.csv'
...
for each_url in course_links_file:
    ...
    try:
        ...
        if svg_div:
            ...
            if unit_tag:
                ...
                print('Unit: ', unit_val)
                local_fee = int(unit_val) * (int(cleaned_fee)/int(36))  # formula
    ...
    # SUBJECTS
    try:
        THE_XPATH = "//td[@data-header='Course name']/*/a"
        ...
        a_tags = values
        subjects_links = []
        domain_url = "https://study.unisa.edu.au/"
        delay = 2
        for a in a_tags:
        ...
        for sl in subjects_links:
            ...
            try:
                THE_XPATH = "//*[text()='Course content']/following::*[1]"
                WebDriverWait(browser, delay).until(
                    EC.presence_of_all_elements_located(
                        (By.XPATH, f'{THE_XPATH}'))
                )
            ...
            try:
                THE_XPATH = "//*[text()='Course aim']/following::*[1]"
                WebDriverWait(browser, delay).until(
                    EC.presence_of_all_elements_located(
                        (By.XPATH, f'{THE_XPATH}'))
                )
