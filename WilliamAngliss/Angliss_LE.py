...
course_type_links = []
course_links = []
each_url = 'https://www.angliss.edu.au/courses/'
domain_url = 'https://www.angliss.edu.au/'
browser.get(each_url)
pure_url = each_url.strip()
each_url = browser.page_source

soup = bs4.BeautifulSoup(each_url, 'lxml')
a_tags = browser.find_elements_by_xpath("//div[@class='container']/div[@class='row']/a")
for a in a_tags:
    ...
    if link:
        ...
        print(link_)
for website in course_type_links:
    browser.get(website)
    webpage = browser.page_source
    a_tags = browser.find_elements_by_xpath("//section[@class='s-tile-pile']/div/div/a")
    for a in a_tags:
        link = a.get_attribute('href')
        if link:
            link_ = urljoin(domain_url, link)
...
course_links_file_path = os.getcwd().replace('\\', '/') + '/angliss_links_file'
