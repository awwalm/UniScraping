...
with open('LocalData/2020_INTL_Indicative_Fees_-_UG_-_7.8.20.html', encoding='utf8') as dom_file:  # Domestic Fees local HTML
...
course_links_file_path = course_links_file_path.__str__() + '/western_sydney_ug_links_file'
course_links_file = open(course_links_file_path, 'r')

# the csv file we'll be saving the courses to
csv_file_path = Path(os.getcwd().replace('\\', '/'))
csv_file = 'WesternSydney_All_Data.csv'
...
for each_url in course_links_file:
    ...
    # ATAR
    course_data['Prerequisite_1'] = str()
    try:
        THE_XPATH = "(//*[text()='ATAR'][1]/following::*[1])[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        ...
        logging.warning(e)
    if len(course_data['Prerequisite_1']) < 4:
        try:
            atar_tag = soup.find('dt', text='ATAR').find_next_sibling('dd')
    ...
    # FEES
    try:
        FEE_DROPDOWN_XPATH = "//select[@class='tabs__select js-tabs__select']/option[contains(text(), 'International')]/ancestor::*[1]"
        INT_FEE_XPATH = "//option[contains(text(), 'International Student')]"
        THE_XPATH = "//b[text()='Fees:']/ancestor::*[1][contains(text(), '$')]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{FEE_DROPDOWN_XPATH}'))
        )
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{INT_FEE_XPATH}'))
        )
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        browser.find_element_by_xpath(f'{FEE_DROPDOWN_XPATH}').click()
        browser.find_element_by_xpath(f'{INT_FEE_XPATH}').click()
        value = browser.find_element_by_xpath(f'{THE_XPATH}').text
    ...
    try:
        int_fee_tag = soup.find('p', {'text': re.compile('^.*AUD.*\$.*$')})
        print(tag_text(int_fee_tag))
    except AttributeError:
        pass

    name_course = course_data['Course']
    head, sep, tail = name_course.partition(' (')
    try:
        THE_XPATH = f"//*[contains(text(), '{head}')]/ancestor::tr/td/*[contains(text(), '$')]"
        ...
        print('cant extract fees (direct file)')
