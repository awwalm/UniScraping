...
browser2.get("https://www.westernsydney.edu.au/future/study/application-pathways/the-college/study-with-us/how-to-apply-international/fees.html")
time.sleep(1)
...
course_links_file_path = course_links_file_path.__str__() + '/western_sydney_col_links_file'
course_links_file = open(course_links_file_path, 'r')

# the csv file we'll be saving the courses to
csv_file_path = Path(os.getcwd().replace('\\', '/'))
csv_file = 'WesternSydney_COL_Data.csv'
...
for each_url in course_links_file:
    ...
    try:
        ...
        logging.warning(e)
        print('cant find fees tag')
    if len(course_data['Int_Fees']) < 2:
        try:
            keywords = browser2.find_elements_by_xpath("(//table[@id='tablePreview'])[3]/tbody/tr/td[1]")
            for i in keywords:
                if i.text in course_data['Course']:
                    THE_XPATH = f"(//table[@id='tablePreview'])[3]/tbody/tr/td[contains(text(), '{i.text}')]/following-sibling::*[2]"
            ...
            print('cannot extract int fees (browser2)')
