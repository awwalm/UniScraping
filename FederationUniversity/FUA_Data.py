...
sample = ['https://study.federation.edu.au/course/DHN9.RP']
...
for each_url in course_links_file:
    ...
    # SUBJECTS
    try:
        THE_XPATH = "//div[contains(@class, 'sample') and contains(@class, 'structure') and contains(@class, '-')]/div"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        element = browser.find_element_by_xpath("//h2[contains(text(), 'Course content')]")
        element.click()
        ...
        try:
            element = browser.find_element_by_xpath("//h2[contains(text(), 'Sample structure')]")
            element.click()
            THE_XPATH = "//div[contains(@class, 'sample') and contains(@class, 'structure') and contains(@class, '-')]/div/a[@href]"
            ...
            subjects_links = []
            domain_url = "https://study.federation.edu.au/"
            delay = 3
            for a in a_tags:
            ...
            for sl in subjects_links:
                ...
                try:
                    THE_XPATH = "//div[@class='subject-info-paragraph']"
                    WebDriverWait(browser, delay).until(
                        EC.presence_of_all_elements_located(
                            (By.XPATH, f'{THE_XPATH}'))
                    )
