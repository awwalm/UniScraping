...
course_links_file_path = course_links_file_path.__str__() + '/excelsia_links_file'
course_links_file = open(course_links_file_path, 'r')

# the csv file we'll be saving the courses to
csv_file_path = Path(os.getcwd().replace('\\', '/'))
csv_file = 'Excelsia_All_Data.csv'
...
for each_url in course_links_file:
    ...
    course_data = {'Level_Code': '', 'University': 'Excelsia College - Sydney', 'City': '', 'Course': '',
                   'Faculty': '',
                   'Int_Fees': '', 'Local_Fees': '', 'Currency': 'AUD', 'Currency_Time': 'Units', 'Duration': '',
                   'Duration_Time': '', 'Full_Time': 'Yes', 'Part_Time': 'No',
                   'Prerequisite_1': '', 'Prerequisite_1_grade_1': '',  # ATAR
    ...
    req_url = urljoin(pure_url, '/where-will-this-take-me')
    ...
    # FEES
    try:
        IN_COURSE_NAME = course_data['Course'][3:].upper()
        THE_XPATH = f"(//*[contains(text(), '{IN_COURSE_NAME}')]/ancestor::tr/td[last()]/*)[1]"
        ...
        print('cannot extract int fees')
    try:
        IN_COURSE_NAME = course_data['Course'][3:].upper()
        THE_XPATH = f"(//*[contains(text(), '{IN_COURSE_NAME}')]/ancestor::tr/td[last()-1]/*)[1]"
        print(IN_COURSE_NAME)
        value = browser2.find_element_by_xpath(f'{THE_XPATH}').text
        course_data['Local_Fees'] = value.replace('$', '').replace(',', '')
    except (AttributeError, TimeoutException, NoSuchElementException, ElementNotInteractableException):
        print('cannot extract local fees')

    # DESCRIPTION
    try:
        THE_XPATH = "(//div[@class='panel acc-group'])[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
        ...
        try:
            THE_XPATH = "(//div[@class='znColumnElement-innerContent'])[3]"
            WebDriverWait(browser, delay).until(
                EC.presence_of_all_elements_located(
                    (By.XPATH, f'{THE_XPATH}'))
            )
    ...
    try:
        THE_XPATH = "//td[contains(text(), 'IELTS')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
    ...
    # PREREQUISITE 3 (AQF)
    try:
        THE_XPATH = "//td[contains(text(), 'AQF')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        value = browser.find_element_by_xpath(f'{THE_XPATH}').text
        course_data['Prerequisite_3'] = 'AQF Level'
        course_data['Prerequisite_3_grade_3'] = re.findall(r'[-+]?\d*\.\d+|\d+', value)[0]
    except (AttributeError, TimeoutException, NoSuchElementException, ElementNotInteractableException):
        print('cant extract aqf')

    # DELIVERY
    try:
        THE_XPATH = "//td[contains(text(), 'Delivery')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        value = browser.find_element_by_xpath(f'{THE_XPATH}').text
        course_data['Online'] = \
            'Yes' if 'online' in value.lower() else course_data['Online']
        course_data['Offline'] = \
            'Yes' if 'on campus' in value.lower() else course_data['Offline']
        course_data['Face_to_Face'] = course_data['Offline']
    except (AttributeError, TimeoutException, NoSuchElementException, ElementNotInteractableException):
        print('cant extract delivery mode')

    # AVAILABILITY
    try:
        THE_XPATH = "//td[contains(text(), 'Available to')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        value = browser.find_element_by_xpath(f'{THE_XPATH}').text
        course_data['Availability'] = \
            'A' if 'International' in value and 'Domestic' in value else course_data['Availability']
        course_data['Availability'] = \
            'I' if 'International' in value and 'Domestic' not in value else course_data['Availability']
        course_data['Availability'] = \
            'D' if 'International' not in value and 'Domestic' in value else course_data['Availability']
    ...
    # DURATION
    try:
        THE_XPATH = "//td[contains(text(), 'Length')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        ...
        try:
            THE_XPATH = "//td[contains(text(), 'Course Duration')]/following::*[1]"
            WebDriverWait(browser, delay).until(
                EC.presence_of_all_elements_located(
                    (By.XPATH, f'{THE_XPATH}'))
            )
    ...
    try:
        WebDriverWait(browser, delay).until(
            EC.element_to_be_clickable(
                (By.XPATH, "(//*[contains(text(), 'WHERE WILL THIS TAKE ME')])[1]/ancestor::a[1]"))
        )
        browser.get(req_url)
        each_url = browser.page_source
        soup = bs4.BeautifulSoup(each_url, 'lxml')
        try:
            THE_XPATH = "//strong[contains(text(), 'WHERE WILL')]/ancestor::div[1]"
            WebDriverWait(browser, delay).until(
                EC.presence_of_all_elements_located(
                    (By.XPATH, f'{THE_XPATH}'))
            )
            ...
            try:
                THE_XPATH = "(//div[@class='znColumnElement-innerContent'])[last()]"
                WebDriverWait(browser, delay).until(
                    EC.presence_of_all_elements_located(
                        (By.XPATH, f'{THE_XPATH}'))
                )
    ...
    print('AQF: ', course_data['Prerequisite_3_grade_3'])
