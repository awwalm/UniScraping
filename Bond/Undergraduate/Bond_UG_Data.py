...
def text_from_html(body_):
    _soup_ = bs4.BeautifulSoup(body_, 'html.parser')
    texts = _soup_.findAll(text=True)
...
course_links_file_path = course_links_file_path.__str__() + '/bond_ug_links_file'
course_links_file = open(course_links_file_path, 'r')

# the csv file we'll be saving the courses to
csv_file_path = Path(os.getcwd().replace('\\', '/'))
csv_file = 'Bond_UG_Data.csv'
...
for each_url in course_links_file:
    ...
    # LOCAL FEES 2020
    try:
        fee_tag = soup.find('strong', text=re.compile('Semester program fee:', re.IGNORECASE))\
    ...
    # INT FEES 2020
    try:
        fee_tag = soup_.find('strong', text=re.compile('Semester program fee:', re.IGNORECASE))\
            .find_parent('p').find_next('ul').find('li')
        if fee_tag:
            fee_tag.strong.decompose()
    ...
    try:
        ...
        if fee_tag:
            fee_tag.strong.decompose()
            int_fees = tag_text(fee_tag).replace('* per semester average', '').replace('$', '').replace(',', '')
            print('int fees 2021: ', int_fees)
    except (AttributeError, IndexError):
        print('2021 int fees not provided')

    # SUBJECTS
    try:
        THE_XPATH = "(//a[@id='structure_and_subjects']/following::div/div[@class='panel-body']/ul/li/a[@target])" \
                    "[position() < last()]"
        ...
        for sl in subjects_links:
            browser.get(sl)
            try:
                THE_XPATH = "//h1[@class='page-title'][1]"
            ...
            try:
                THE_XPATH = "//a[@id='description']/ancestor::*[1]/p"
                WebDriverWait(browser, delay).until(
                    EC.presence_of_all_elements_located(
                        (By.XPATH, f'{THE_XPATH}'))
                )
            ...
            try:
                THE_XPATH = "//a[@id='learning']/ancestor::*[1]/p"
                WebDriverWait(browser, delay).until(
                    EC.presence_of_all_elements_located(
                        (By.XPATH, f'{THE_XPATH}'))
                )
