...
course_links_file_path = course_links_file_path.__str__() + '/anu_undergrad_links_file'
course_links_file = open(course_links_file_path, 'r')

# the csv file we'll be saving the courses to
csv_file_path = Path(os.getcwd().replace('\\', '/'))
csv_file = 'ANU_Undergrad_Data.csv'
...
course_data = {'Level_Code': '', 'University': 'Australian National University', 'City': 'Canberra', 'Course': '', 'Faculty': '',
               'Int_Fees': '', 'Local_Fees': '', 'Currency': 'AUD', 'Currency_Time': 'Years', 'Duration': '',
               'Duration_Time': '', 'Full_Time': '', 'Part_Time': '',
               'Prerequisite_1': 'ATAR', 'Prerequisite_2': 'GPA', 'Prerequisite_3': 'Equivalent ANU AQF Degree Level',
...
for each_url in course_links_file:
    ...
    try:
        ...
        if div1:
            ...
            if div_h2:
                the_div = soup.find('div', class_='callout-box callout-box--reverse callout-box--career callout-box--right').\
                    find('div')
                if the_div:
    ...
    # DURATION
    try:
        div_span = soup.find('div', class_='degree-summary hide-mobile w-doublenarrow').find('div').find('ul').find('li').find_all('span')[1]
        if div_span:
    ...
    if div_ps:
        ...
        try:
            if div_ps[0]:  # ANU AQF LEVEL
                anu_aqf = tag_text(div_ps[0])
                pre_req_3_val = int(list(filter(str.isdigit, anu_aqf))[0])
                course_data['Prerequisite_3_grade_3'] = pre_req_3_val
                print('ANU AQF DEGREE LEVEL: ', pre_req_3_val)
        except IndexError:
            course_data['Prerequisite_3_grade_3'] = ''

        try:
            if div_ps[1]:  # GPA out of 7.0
                gpa = tag_text(div_ps[1])
                if 'GPA' in gpa:
                    pre_req_2_val = float(list(filter(str.isdigit, gpa))[0])
                    course_data['Prerequisite_2_grade_2'] = pre_req_2_val
                    print('GPA out of 7.0: ', pre_req_2_val)
            else:
                for p in div_ps:
                    gpa = tag_text(p)
                    if 'GPA' in gpa:
        ...
        for p in div_ps:
            if 'ATAR' in tag_text(p) and has_numbers(tag_text(p)):
                atar = tag_text(p)
                atar_val1 = int(list(filter(str.isdigit, atar))[0])
                atar_val2 = int(list(filter(str.isdigit, atar))[1])
                atar_val = str(atar_val1) + str(atar_val2)
                if int(atar_val) in range(40, 100):
                    course_data['Prerequisite_1_grade_1'] = atar_val
                break
            else:
                course_data['Prerequisite_1_grade_1'] = ''
        if course_data['Prerequisite_1_grade_1'].strip() == '':
            abbr = soup.find('abbr', title='Australian Tertiary Admission Rank', class_='degree-summary__admission-label')
            if abbr:
                atar_val = abbr.find_next('span').text.__str__().strip()
                course_data['Prerequisite_1_grade_1'] = atar_val
        else:
            course_data['Prerequisite_1_grade_1'] = '70'

    print('ATAR: ', course_data['Prerequisite_1_grade_1'])
    print('GPA: ', course_data['Prerequisite_2_grade_2'])
