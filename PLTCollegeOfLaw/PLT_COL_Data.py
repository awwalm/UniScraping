...
course_links_file_path = course_links_file_path.__str__() + '/plt_col_links_file'
course_links_file = open(course_links_file_path, 'r')

# the csv file we'll be saving the courses to
csv_file_path = Path(os.getcwd().replace('\\', '/'))
csv_file = 'PLT_COL_All_Data.csv'
...
for each_url in course_links_file:
    ...
    course_data = {'Level_Code': '', 'University': 'The College of Law', 'City': '', 'Course': '',
                   'Faculty': '',
    ...
    # COURSE NAME
    title_ = None
    title = soup.find('h1', {'class': 'course-title'})
    if title:
    ...
    # DESCRIPTION
    try:
        desc_tag = soup.find('div', {'class': 'course-description'}).find_all('p')
        if desc_tag:
    ...
    # DELIVERY MODE
    try:
        THE_XPATH = "//ul[@class='course-tags']"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
        ...
        print('cant extract delivery mode tags')

    # FEES
    try:
        THE_XPATH = "//span[@class='price-label' and contains(text(), 'Domestic')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
    ...
    try:
        THE_XPATH = "//span[@class='price-label' and contains(text(), 'International')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        ...
        print('cant extract int fees')

    # OUTCOMES
    try:
        THE_XPATH = "//*[contains(text(), 'Overview')]/ancestor::div[1]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
    ...
    # REMARKS
    try:
        THE_XPATH = "//*[contains(text(), 'Structure')]/ancestor::div[1]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
    ...
    try:
        THE_XPATH = "//p[@class='label course-id']"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        ...
        print('cant extract course code')

    # DURATION
    start_date, end_date = None, None
    try:
        THE_XPATH = "//*[contains(text(), 'Start date')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        value = browser.find_element_by_xpath(f'{THE_XPATH}').text
        x = value.split()
        start_date = dt(int(x[-1]), months[x[1]], int(x[0]))
    except (AttributeError, TimeoutException, NoSuchElementException, ElementNotInteractableException):
        print('cant extract start date')
    try:
        THE_XPATH = "//*[contains(text(), 'End date')]/following::*[1]"
        WebDriverWait(browser, delay).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, f'{THE_XPATH}'))
        )
        value = browser.find_element_by_xpath(f'{THE_XPATH}').text
        x = value.split()
        end_date = dt(int(x[-1]), months[x[1]], int(x[0]))
    except (AttributeError, TimeoutException, NoSuchElementException, ElementNotInteractableException):
        print('cant extract end date')
    if end_date is not None and start_date is not None:
        duration = str((end_date - start_date).days) + " days"
        course_data = convert_duration_cleanly(tagged_text=duration, course_dict=course_data)
        # print(duration)

    # LOCATION
    for i in possible_cities:
        if i in course_data['Course']:
            actual_cities.add(i)
