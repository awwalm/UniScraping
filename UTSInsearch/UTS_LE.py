...
course_type_links = []
course_links = []
each_url = 'https://www.insearch.edu.au/au/programs'
domain_url = 'https://www.insearch.edu.au/'
...
a_tags = browser.find_elements_by_xpath("//a[@class='promo promo__link']")
for a in a_tags:
    link = a.get_attribute('href')
    if link:
        link_ = urljoin(domain_url, link)
        if link_ not in course_type_links:
            course_type_links.append(link_)
            print(link_)

for webpage in course_type_links:
    browser.get(webpage)
    a_tags = browser.find_elements_by_xpath("//a[contains(text(), 'Course details') and @href]")
    for a in a_tags:
        link = a.get_attribute('href')
        if link:
            link_ = urljoin(domain_url, link)
...
course_links_file_path = os.getcwd().replace('\\', '/') + '/uts_links_file'
